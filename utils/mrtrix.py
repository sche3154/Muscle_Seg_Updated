...
class Image (object):
    ...
    def __init__(
        self, data=None, vox=(), transform=np.eye(4),
        grad=None, comments=None, commands=None, version=None
    ):
        self._dtdict = {
            'Int8': '|i1', 'UInt8': '|u1', 'Int16': '=i2', 'UInt16': '=u2',
            'Int16LE': '<i2', 'UInt16LE': '<u2', 'Int16BE': '>i2',
            'UInt16BE': '>u2', 'Int32': '=i4', 'UInt32': '=u4',
            'Int32LE': '<i4', 'UInt32LE': '<u4', 'Int32BE': '>i4',
            'UInt32BE': '>u4', 'Float32': '=f4', 'Float32LE': '<f4',
            'Float32BE': '>f4', 'Float64': '=f8', 'Float64LE': '<f8',
            'Float64BE': '>f8', 'CFloat32': '=c8', 'CFloat32LE': '<c8',
            'CFloat32BE': '>c8', 'CFloat64': '=c16', 'CFloat64LE': '<c16',
            'CFloat64BE': '>c16'
        }
        self._dtdict_inv = {v: k for k, v in self._dtdict.items()}
        self.data = data
        self.vox = vox
        ...
        self._array_attr = ['shape', 'ndim', 'dtype', 'size', 'strides', 'nbytes']
    ...
    def load(self, filename, header_only=False, memmap=False):
        ...
        if memmap:
            if is_gz:
                raise IOError(
                    'memory mapping of .mif.gz files is not supported'
                )
            if header_only:
                raise IOError(
                    'memory mapping and header_only are mutually exclusive'
                )
        ...
        with ofun(filename, 'rt', encoding='latin-1') as f:
            ...
            while fl != 'END':
                ...
                if fl.startswith('dim'):
                    imsize = tuple(
                        map(int, fl.split(':')[1].strip().split(','))
                    )
                    ...
                    dt = np.dtype(self._dtdict.get(dtstr, 'u1'))
                elif fl.startswith('file'):
                    offset = int(fl.split('.')[1].strip())
                elif fl.startswith('transform'):
                    self.transform[tr_count,:] = np.array(
                        fl.split(':')[1].strip().split(','), dtype=float
                    )
                    tr_count = tr_count + 1
                ...
                elif fl.startswith('command'):
                    self.commands.append(fl[17:].strip())
                elif fl.startswith('mrtrix_version'):
                    self.version = fl[16:].strip()
        ...
        if not header_only:
            ...
            with ofun(filename, 'rb') as f:
                if is_gz:
                    ...
                    image = np.memmap(
                        f, mode='r', dtype=dt, offset=offset
                    )  # offset required in memmap
                else:
                    f.seek(offset, 0)
                ...
                self.data = np.ndarray(
                    shape=imsize, dtype=dt, buffer=image, strides=s, offset=o
                )
        return self
    ...
    def save(self, filename):
        ...
        with open(filename, 'w', encoding='latin-1') as f:
            ...
            f.write(
                'layout: ' + self._to_csv(self.layout, precision='%s') + '\n'
            )
            f.write(
                'datatype: ' + self._dtdict_inv[self.dtype.descr[0][1]] + '\n'
            )
            f.write(self._to_csv2D(self.transform[:3], 'transform: '))
            if len(self.commands) > 0:
                f.write(
                    'command_history: %s\n' * len(self.commands) % tuple(self.commands)
                )
            if self.labels is not None:
                f.write(
                    'labels: ' + self._to_csv(self.labels, precision='%s') + '\n'
                )
            if self.units is not None:
                f.write(
                    'units: ' + self._to_csv(self.units, precision='%s') + '\n'
                )
            if self.comments:
                f.write(
    ...
    def layout(self):
        ...
        limitations.
        """
        sorted_strides = np.argsort(np.argsort(np.abs(self.strides)))
        return tuple('+'+str(s) for s in sorted_strides)
    ...
    def _to_csv(self, a, precision=None):
        ...
        if precision is None:
            ...
            raise ValueError(
                'precision needs to be non-negative integer, got ' + str(precision)
            )
    ...
    def _to_csv2D(self, a, line_prefix='', postfix='\n', precision=None):
        ...
        if a.ndim != 2:
            raise ValueError(
                'require 2D array, got array of shape ' + str(a.shape)
            )
        if precision is None:
            if isinstance(a, np.ndarray):
                precision = np.finfo(a.dtype).precision
    ...
    def __str__(self):
        ...
        if self.data is not None:
            ...
            out += '  datatype: ' + self._dtdict_inv[self.dtype.descr[0][1]] + '\n'
